...
- `Highlight` — startSec, endSec, score, title (자막에서 파생, 저장하지 않음)
...
- **후보 목록 검색은 별도 유스케이스**: `SearchLyricsCandidatesHandler`는 활성 출처를 **전부** 훑어 후보(`LyricsCandidateDTO`)를 모으기만 하고 **애그리게이트를 건드리지 않는다**(읽기 전용). 출처당 후보는 여러 건이며(같은 제목·다른 가수), 다건 조회는 선택 포트 `ILyricsSearchProvider.search`로 분리해 미구현 출처는 `ILyricsProvider.fetch` 1건으로 폴백한다. 반영은 사용자가 고른 뒤 `ApplyLyricsCandidateHandler`가 `SongInfoAggregate.apply_fetched(force_lyrics=True)`로 수행한다 — "조회"와 "상태 변경"을 분리해, 목록을 띄우는 것만으로 저장된 가사가 바뀌지 않게 한다. 검색 기준값·아티스트 폴백·번역 줄 생성은 체인과 같은 모듈 함수를 공유한다.
...
| `GetHighlightsQuery` | 자막에서 볼 만한 구간 제안(순수 계산, 네트워크 없음) |
