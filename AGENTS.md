...
| File | Description |
|------|-------------|
| `main.py` | 진입점 — **조립 목록이 아니라 순서만** 담는다(127줄). 조립은 `bootstrap/`에 있다 |
...
| Directory | Purpose |
|-----------|---------|
| `bootstrap/` | **조립 루트(composition root)** — 리포지토리·서비스·핸들러·뷰모델 조립 + 시작·종료 절차 |
...
| `docs/` | 패키징 가이드·사양 문서 + `architecture/`(파일 맵·설계 결정) (see `docs/AGENTS.md`) |

## For AI Agents

### Working In This Directory
- **조립은 `bootstrap/`에서 한다.** 새 핸들러·어댑터를 추가하면 해당 컨텍스트 파일
  하나만 고친다(`bootstrap/handlers/<context>.py`). `main.py`는 순서만 담으므로
  대개 손대지 않는다. 조립 그래프는 `tests/integration/test_composition_root.py`가 검증한다.
