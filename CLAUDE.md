...
│   │   ├── video_detail_panel.py    # YouTube 시청 페이지형 상세화면 — 좌(상단 고정 플레이어 → **제목 행**(제목 `_title_lbl` + 우측 정렬 아이콘 `⟳`상세갱신·`🌐`브라우저) → 스크롤 정보(채널/메타/상태/태그/챕터/설명) → **메모 섹션**(설명 아래 영속 `_notes_edit`, `_meta_layout` 밖) → **하단 탭 2개**) | 우(`_RelatedList` 연관영상). 탭: `_TAB_FILES`(다운로드/클립 병합 — 수직 `QSplitter`, 위=`_dl_tab` 아래=`_clip_tab_widget`)·`_TAB_SUMMARY`(요약, 헤더 행에 `⟳` 아이콘 갱신 버튼). `load`(로컬)/`load_stream`(스트리밍: 요약 탭+제목행 `⟳` 비활성) + `set_related`. 제목은 `_build_info`가 `_meta_layout`을 `_clear_layout`(deleteLater)로 재빌드하므로 스크롤 밖 `_title_lbl.setText()`로 세팅(영속 위젯을 `_meta_layout`에 두지 말 것). 설명에서 `_parse_chapters`로 타임스탬프 추출→클릭 시 `InlinePlayer.seek_to_ms`. `RelatedItem` dataclass + `item_selected` 시그널. 요약 탭은 `gemini_summary` 필드를 표시(`_render_summary_html`로 타임스탬프를 seek 링크화)하며 ⟳ 버튼으로 `_GeminiSummaryWorker`(QThread) → `GeminiExtractor` 호출 → `gemini_summary_saved` 방출. 제목행 `⟳`는 `detail_refresh_requested(video_id)` 방출 → `LibraryPanel._on_detail_refresh_requested`가 `get_video_detail` 재조회 후 `load` 재호출(nav 히스토리 미변경).
