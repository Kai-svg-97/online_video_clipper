...
│       ├── recording_repository.py  # RecordingVideoRepository(video save/delete + category save/delete[origin-identity] + video_tag 링크 diff 캡처)·RecordingSongRepository(song_info save/delete, nkey=영상 URL). 나머지(playlist·clip·download)는 Phase D-2b
│       ├── merge_applier.py         # MergeApplier — OpLogMerger로 승자 계산 후 자연키→UUID 해석·FK 재작성·위상 순서로 라이브 DB 직접 반영(FTS 트리거 정상 발화). 핸들러 registry: Category(origin-identity, name+parent ref, rename=필드변경, 동명충돌 병합)·Video·Song(nkey=영상URL)·VideoTag(link_key, 태그는 resolve_tag로 lazy 생성) ApplyHandler + resolve_video/resolve_tag/resolve_category(origin nkey→stub 생성). 배치 내 부모/자식 순서 무관하도록 resolve_category가 스텁을 만들고 실제 op이 채움
