...
│   │   ├── settings_panel.py        # 전체 설정 패널 (다운로드 경로, 테마 등) + **가사 출처 관리**(`_LyricsSourcesSection`: `song_vm` 주입 시에만 표시) + **클라우드 동기화**(`_CloudSyncSection`: 제공자 드롭다운(**로컬 폴더**(기본)/Google Drive/OneDrive) — 폴더 선택 시 경로 입력+찾아보기, API 선택 시 Client ID/Secret 입력·연결/해제/지금 동기화 버튼·상태 라벨, `sync_vm` 주입 시에만 표시)
