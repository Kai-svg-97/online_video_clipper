...
│   │   ├── stats_panel.py           # 라이브러리 통계 대시보드 + **채널별 카테고리 섹션**(`_make_channel_row`: 채널명·총 영상수 + 카테고리 경로 링크를 `_FlowLayout`으로 흐름 배치, 예 "IT > News (3)"). 링크 클릭 시 `category_selected(category_id)` 방출 → `MainWindow._on_stats_category_selected`가 라이브러리 해당 카테고리로 전환. 데이터는 `LibraryStatsDTO.channel_stats`(list[`ChannelStatDTO`]→`ChannelCategoryStatDTO`)
