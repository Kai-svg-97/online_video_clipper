...
- **앨범 보기 (음악 카테고리 전용, 파생 그룹)** — 최상위 카테고리가
  `MUSIC_ROOT_CATEGORY_NAMES`(music·song·음악·노래·뮤직)일 때만 정렬 콤보에 **'앨범'**이
  추가된다(`LibraryPanel._update_sort_options`). 이 항목은 SQL 정렬 컬럼이 아니라 **화면 모드
  전환**(`_SORT_ALBUM` sentinel)이라 리포지토리로 넘기지 않는다 — 넘어가면 조회가 깨진다.
  ...
  **iTunes `lookup`에는 `country`를 붙이면 안 된다** — 실측 결과 수록곡이 통째로 빠지고 앨범
  ...
  `tests/gui/test_album_view.py`(정렬 노출 조건·배지·재생 배선).
