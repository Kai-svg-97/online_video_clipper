...
│       └── sync_service.py          # SyncService — sync 스택 조립(secret·device·clock·store·applier·snapshot·recorder·provider) + 고수준 동작(sync_now·sync_media·compact·connect_gdrive/onedrive·disconnect·status·make_recording_repos). pre_db_bootstrap()·build_secret_store()·build_provider() 모듈 함수. **캡처 게이팅**: is_connected일 때만 make_recording_repos가 Recording* dict 반환(미연결이면 None→repo 무래핑)
│
...
│   │   ├── settings_panel.py        # 전체 설정 패널 (다운로드 경로, 테마 등) + **가사 출처 관리**(`_LyricsSourcesSection`: `song_vm` 주입 시에만 표시) + **클라우드 동기화**(`_CloudSyncSection`: 제공자 드롭다운(Google Drive/OneDrive)·Client ID/Secret 입력·연결/해제/지금 동기화 버튼·상태 라벨, `sync_vm` 주입 시에만 표시)
