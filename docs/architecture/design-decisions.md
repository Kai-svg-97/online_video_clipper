...
#### 메뉴가 뜨는 것 자체는 자동 검증이 안 된다

여기까지 확인할 수 있다: `WM_NCHITTEST`가 최대화 버튼에서 `HTMAXBUTTON`을 돌려주는가
(패키징된 exe에 `SendMessage`로 직접 물어 확인), 호버·클릭이 비클라이언트 메시지를 타고
버튼에 되돌아오는가.

**메뉴가 실제로 떴는지는 확인할 수 없었다.** `SetCursorPos`도 `SendInput`도 셸의 호버
감지를 깨우지 못했다. 대조군으로 **메모장**(네이티브 타이틀바, 스냅이 확실히 동작)을 같은
방법으로 훑어 봤는데 거기서도 메뉴가 뜨지 않았다 — 즉 우리 창의 문제가 아니라 합성 입력의
한계다. 메뉴 등장은 사람이 직접 마우스를 올려 봐야 한다.

그래서 회귀 테스트가 지키는 것은 **히트테스트 값과 메시지 왕복**까지다. 그 둘이 맞으면
셸이 해 줄 일은 셸의 몫이고, 실제로 깨질 수 있는 쪽(우리가 쓴 코드)은 거기까지다.
