...
번역기(`ITranslator`)는 **한 줄씩** 호출하는 설계다 — 가사 40줄에 맞춰 만들었고,
줄마다 독립적으로 try/except해 한 줄의 실패가 나머지에 번지지 않게 돼 있다. 자막은
수백~수천 줄이라 그대로 쓰면 왕복이 그만큼 나고, 몇 분이 걸리며 백엔드가 막기도 한다.

그래서 40줄씩 줄바꿈으로 이어 한 번에 보낸다(500줄이면 13번 왕복). **대신 정렬이
깨질 위험이 생긴다** — 번역기가 줄을 합치거나 나누면 원문과 번역의 짝이 어긋나고,
그러면 엉뚱한 시점에 엉뚱한 말이 뜬다. **틀렸다는 티조차 나지 않는 실패**라 가장
고약하다.

안전장치는 단순하다: 묶음마다 **돌아온 줄 수를 센다**. 맞지 않으면 `split_chunk`가
`None`을 돌려주고, 그 묶음만 한 줄씩 다시 한다. 억지로 맞추지 않는다.

곁가지 규칙 둘:

* **실패한 자리는 원문을 남긴다.** 빈 줄로 두면 그 시점에 자막이 사라져, 번역 실패인지
  원래 말이 없는 것인지 알 수 없다.
* **결과가 원문과 똑같으면 저장하지 않는다.** 번역기가 통째로 실패한 경우인데, 그걸
  '번역본'이라고 저장하면 목록만 늘고 쓸모는 없다.

원본을 덮지 않고 `tr-ko`라는 별도 언어 키로 저장하는 것은 음성 인식(`asr-auto`)과
같은 판단이다 — 원문으로 검색하고 싶을 수도, 번역이 엉망일 수도 있다.
...
업데이트가 **사용자에게 거의 보이지 않았다.** 새 버전을 찾으면 앱이 조용히 179MB를
받고, 알림은 좌하단 기어의 빨간 점 하나였다. 그 점의 뜻을 알려면 설정 화면까지
들어가야 했고, 설치는 "앱을 닫으면 자동으로" 이뤄졌다 — 사용자가 직접 앱을 닫기
전까지는 영원히 일어나지 않는다.

### "앱이 켜진 채로 설치"는 불가능하다

실행 중인 `.exe`와 `_internal/*.dll`은 Windows가 잠근다. 파일 교체 순간에는 앱이
반드시 내려가 있어야 한다. 그래서 목표를 **공백 최소화**로 바꿨다: 다운로드는 앱이
켜진 채 끝내고, 배치가 **우리 PID가 사라지는 것을 감시하다가** 죽는 즉시 설치를
시작하며, `/SILENT`로 인스톨러 진행 막대를 띄워 공백 동안에도 화면에 무언가를 남긴다.

### 이 작업에서 드러난 기존 버그 셋

세 가지 모두 **조용히 실패해서 아무도 몰랐던** 것이다.

1. **`timeout /t 5`가 한 번도 기다린 적이 없다.** 배치를 `DETACHED_PROCESS`로 띄우면
   콘솔이 없고, 콘솔이 필요한 명령은 즉시 실패한다. "5초 대기"라고 적혀 있었을 뿐
   실제로는 0초였고, 파일 잠금을 막아 준 것은 인스톨러의 `CloseApplications=force`
   뿐이었다. `CREATE_NO_WINDOW`로 바꿔 창은 숨기되 콘솔은 준다.
2. **`find`가 GNU find로 잡힌다.** Git for Windows가 깔린 PC에서는 그쪽
   `usr/bin/find.exe`가 PATH 앞에 있고, 인자를 파일 경로로 해석해 실패한다. 그러면
   `|| goto ready`가 발동해 **PID 대기가 통째로 사라진다.** 시스템 도구는 절대 경로로
   부르고, 이름이 겹치지 않는 `findstr`을 쓴다.
3. **배치 파일 인코딩.** 사용자 이름이 한글이면 임시 경로에 한글이 섞이는데(한국어
   사용자에게 흔하다) ascii로 기록하면 실패해 업데이트가 통째로 막힌다. 시스템 ANSI
   코드페이지로 기록하되, **우리가 적는 명령어는 ASCII만** 쓴다 — 반대로 배치에 한글
   주석을 넣으면 영문 Windows에서 기록이 실패한다.

### 발견은 인터벌을 소진하지 않는다

다운로드를 "발견 즉시"에서 "누를 때"로 옮기면서 `_mark_checked()` 위치가 문제가 됐다.
발견 시점에 1시간 인터벌을 소진하면 **앱을 껐다 켰을 때 확인을 건너뛰어 배지가 뜨지
않고, 사용자는 업데이트할 방법을 잃는다.** 이 저장소가 이미 한 번 밟은 함정이라
(`tests/gui/test_update_flow.py` 상단 회귀 배경) 발견은 종결된 결과로 치지 않는다.
대가로 업데이트가 대기 중이면 1시간마다 재확인하는데, 그 사이 더 새 버전이 나오면
배지가 그쪽으로 갱신되므로 의도된 동작이다.

### 커스텀 타이틀바 — 왜 네이티브 훅인가

OS 제목 표시줄에는 위젯을 넣을 수 없어 frameless로 갔다. 단순히
`Qt.FramelessWindowHint`만 주면 **Aero Snap·창 그림자·8방향 리사이즈·최대화
애니메이션·`Win+화살표`·시스템 메뉴가 전부 사라진다.** 특히 "최대화 상태에서
드래그하면 복원되며 커서를 따라오는" 동작은 DPI가 다른 모니터 사이에서 수식으로
맞추기가 사실상 불가능하다.

그래서 `WS_THICKFRAME | WS_CAPTION`을 **남겨 둔 채** `WM_NCCALCSIZE`로 프레임 영역만
0으로 만든다. 위 동작이 전부 OS 기본으로 들어오고, 우리가 할 일은 `WM_NCHITTEST`에서
"여기는 제목 표시줄"이라고 답해 주는 것뿐이다 — **드래그 코드를 한 줄도 쓰지 않는다.**

이 방식의 대표적 함정 둘을 함께 처리했다. 최대화 시 프레임 두께만큼 되줄이지 않으면
클라이언트가 "작업영역 + 프레임 두께"를 통째로 가져가 사방이 화면 밖으로 나가고
(우측 스크롤바가 잘린다), 자동숨김 작업표시줄이 있는 변을 100% 덮으면 마우스를 대도
표시줄이 다시 나오지 않는다(2px 양보). 실측 결과 최대화 창이 작업영역과 **사방 넘침
0**으로 일치한다.

### `super().nativeEvent()`가 프로세스를 죽인다

PyQt 6.11에서 파이썬 오버라이드가 기본 구현을 호출하면 **액세스 위반으로 앱이 즉시
사라진다**(0xC000041D, 창이 만들어지는 바로 그 순간). 화면에 아무것도 안 나오고 로그도
없어 원인을 찾기 어렵다 — 최소 재현으로 좁혀서야 드러났다. `return False, 0`이 의미가
같고(Qt 기본 구현도 "처리하지 않았다"만 반환한다) 안전하다. 누가 "기본 구현도 불러야
맞지 않나" 하고 되돌리는 것을 `tests/gui/test_frameless_guard.py`가 AST로 막는다.

### Win11 분할 배치(스냅 레이아웃)는 공짜가 아니다

최대화 버튼 위에서 `HTMAXBUTTON`을 돌려주면 Windows 11 이 분할 배치 메뉴를 띄워 준다.
대신 **그 영역의 마우스를 OS 가 가져간다** — Qt 의 `enterEvent`/`leaveEvent`도, 클릭도
더 이상 오지 않는다. 그래서 비클라이언트 마우스 메시지 4종을 직접 받아 호버와 클릭을
버튼에 되돌려 줘야 한다. 이 처리를 빼면 **메뉴는 뜨는데 버튼이 먹통이 되는** 상태가
되고, 화면만 봐서는 원인을 알 수 없다(그래서 v1.30.0에서는 범위 밖으로 미뤄 뒀다).

호버를 켤 때 `TrackMouseEvent(TME_NONCLIENT|TME_LEAVE)`를 함께 걸어야 한다. 안 걸면
`WM_NCMOUSELEAVE`가 오지 않아 **버튼 밖으로 나가도 호버가 켜진 채 남는다.**

누름(`WM_NCLBUTTONDOWN`)은 삼키고 토글은 **뗄 때** 한다. 기본 처리에 맡기면 OS 가 자기
방식으로 버튼을 그리려 하고, 뗄 때 토글하는 편이 "눌렀다가 밖에서 떼면 취소"라는
실제 버튼 동작과도 맞는다.

#### 히트테스트가 맞아도 메뉴는 안 뜰 수 있다 — `WS_POPUP`

`HTMAXBUTTON`을 정확히 돌려주는데도 분할 배치 메뉴가 뜨지 않았다. 메모장과 창 스타일을
나란히 놓고서야 보였다 — 차이가 **딱 한 비트**였다.

    메모장   0x14CF0000
    우리 앱  0x96CF0000   ← WS_POPUP(0x80000000)

Qt가 `FramelessWindowHint` 때문에 창을 팝업으로 만드는데, **셸은 팝업 창을 평범한 앱
창으로 보지 않아 메뉴를 붙이지 않는다.** 히트테스트가 아무리 맞아도 소용없다.
`patched_style()`이 이 비트를 끄고, `TestPatchedStyle`이 다시 켜지는 것을 막는다.

#### `DwmExtendFrameIntoClientArea` 가 분할 가이드를 막는다

`WS_POPUP` 을 끈 뒤에도 가이드가 뜨지 않았다. 창 스타일·확장 스타일·클래스 스타일·
DWM 속성을 메모장과 전부 대조했지만 의미 있는 차이가 없었다.

원인은 **우리가 그림자를 살리려고 넣은 호출**이었다. 프레임을 클라이언트 안으로
확장하면(`DwmExtendFrameIntoClientArea`) 셸이 분할 배치 메뉴를 띄우지 않는다.
`DwmSetWindowAttribute(DWMWA_NCRENDERING_POLICY, ENABLED)` 는 그대로 둬도 되고,
`WS_THICKFRAME | WS_CAPTION` 만으로도 DWM 이 그림자를 그려 준다.

실측(최대화 버튼 아래 영역의 픽셀 변화율, 메모장 0.839 가 기준):

| 시도 | 변화율 | |
|---|---|---|
| 현재(프레임 확장 유지) | 0.003 | 안 뜸 |
| 위쪽 프레임 1px 남기기 | 0.003 | 안 뜸 |
| **프레임 확장 제거** | **0.844** | **뜸** |
| 위쪽만 확장 `(0,0,1,0)` | 0.003 | 안 뜸 |

#### 가이드가 떴는지 **자동으로** 확인하는 법

앞서 "자동 검증이 안 된다"고 적었는데, 방법이 틀렸던 것이다. 창 목록(`EnumWindows`)
비교로는 메모장에서도 못 잡았다 — 가이드가 일반적인 최상위 창으로 잡히지 않는다.

**최대화 버튼 아래 영역을 `BitBlt` 로 찍어 호버 전후를 비교**하면 잡힌다. 창의 종류를
알 필요가 없고, 뜨면 그 영역의 80% 넘는 픽셀이 바뀐다. 커서는 `SendInput` 으로
조금씩 움직이고(순간이동은 셸의 호버 감지를 못 깨운다), 창을 최상위로 올려야 한다
(가려져 있으면 마우스 메시지가 우리에게 오지 않는다).

**대조군을 먼저 통과시키는 것이 핵심이다.** 메모장에서 감지되지 않으면 그 실행은
버리고 방법부터 고쳐야 한다 — 그러지 않으면 "안 뜬다"가 기능 문제인지 감지 문제인지
알 수 없고, 실제로 그 혼동 때문에 한 번 엉뚱한 결론을 냈다.

#### 최대화를 잴 때는 **클라이언트** 사각형을 본다

`WM_NCCALCSIZE`가 정하는 것은 창 사각형이 아니라 **클라이언트 사각형**이다. 창 사각형만
재면 최대화된 창이 작업영역보다 사방으로 커 보이는데, 그것이 정상이다 — Windows 는
`WS_THICKFRAME` 창을 최대화할 때 창 사각형을 "작업영역 + 프레임 두께"로 잡는다.

실측(4K/150% 화면, 작업영역 3840×2088):

| | 창 | 창 넘침 | 클라이언트 | 클라 넘침 |
|---|---|---|---|---|
| 메모장 | 3862×2110 | 11,11,11,11 | 3844×2101 | 2,11,2,2 |
| 우리 앱 | 3862×2110 | 11,11,11,11 | 3840×2088 | **0,0,0,0** |

창 넘침은 메모장과 **같다**. 우리 클라이언트가 0인 것은 프레임을 지우고 그 자리까지
직접 그리기 때문이고, 이것이 의도한 모습이다.

한 번 이걸 혼동해 "`Win+↑`·타이틀바 더블클릭 경로에서만 11px 넘친다"고 오진하고,
`GetWindowPlacement`가 `WM_NCCALCSIZE` 시점에 낡았을 것이라 추정해 판정을 사각형
기반으로 갈아엎었다. 변경 **전** 빌드로 클라이언트를 재 보니 이미 `0,0,0,0`이었다 —
버그가 없었고 가설도 거짓이었다. 그래서 되돌렸다.

교훈: **고치기 전에 고장을 먼저 재라.** 그리고 재는 대상이 맞는지부터 확인하라 —
정상 동작하는 앱(메모장)과 나란히 놓으면 5분이면 갈린다.

#### 이 버그를 늦게 찾은 이유 — 감지 방법이 틀렸다

히트테스트도 맞고 호버·클릭 왕복도 되니 **자동 검증은 전부 초록이었다.** 메뉴만 안 떴다.

메뉴 등장을 자동으로 확인하려 했지만 `SetCursorPos`도 `SendInput`도 셸의 호버 감지를
깨우지 못했다. 대조군으로 메모장을 같은 방법으로 훑었더니 **거기서도 안 떴고**, 그래서
"합성 입력의 한계이지 우리 문제가 아니다"라고 결론 냈다 — 그 결론이 진짜 버그를 덮었다.
대조군이 실패한 이유는 맞았지만, 그것으로 우리 창의 무죄를 증명할 수는 없었다.

교훈: **관찰할 수 없다는 것이 정상이라는 뜻은 아니다.** 검증 수단이 막혔을 때는
'확인 못 함'으로 남겨야지 '문제 없음'으로 바꿔 읽으면 안 된다. 그리고 막혔을 때는
**대상을 정상 동작하는 것과 나란히 비교**하는 쪽이 빠르다 — 스타일 비교 한 번이
합성 입력을 다섯 번 고쳐 보는 것보다 결정적이었다.

메뉴가 실제로 뜨는지는 여전히 사람이 마우스를 올려 봐야 한다. 대신 회귀 테스트는
**창 스타일·히트테스트 값·메시지 왕복** 셋을 지킨다.

### 배지 글자는 두 번 그린다

진행률 채움 경계가 글자 한가운데를 지나가므로 펜 하나로는 어느 쪽이든 묻힌다.
처음에는 "50% 넘으면 글자색을 바꾼다"로 했는데, 대비를 재 보니 **11개 테마 전부에서**
...
"앱 내 플레이어를 재생하면 늘 해상도가 매우 낮은 것으로 한정적으로 재생된다."

### 원인 — 포맷 선택 버그가 아니라 구조다

기본 화질은 `best[ext=mp4]/best` 였다. yt-dlp에서 `best`는 "가장 좋은 **muxed**
포맷"이고, YouTube가 내주는 muxed는 **itag 18(360p) 하나뿐**이다.

```
[default] total=33 muxed=1
   MUX 18 mp4 360 https avc1 mp4a      ← 영상+소리가 함께 든 포맷은 이것 하나
   video-only heights: [144, 240, 360, 480, 720, 1080, 1440, 2160]
```

즉 원본이 4K여도 이 선택자는 **항상** 360p를 골랐다. "자동 (빠른 재생)"이라는
라벨이 지키지 못할 약속을 하고 있었던 셈이다. `ios`·`tv` 클라이언트로 HLS를
받는 우회로도 확인했으나 지금은 전부 추출 자체가 실패한다.

### 그러면 왜 고화질 URL을 그냥 재생기에 넘기지 않았나

넘길 수 없었다. 같은 1080p URL에 요청 형태만 바꿔 보면:

| 요청 | 응답 |
| --- | --- |
| `Range: bytes=0-` (열린 범위) | **403** |
| `Range: bytes=0-1000` (경계 있는 범위) | 206 |
| `?range=0-1000` (쿼리 파라미터) | 200 |

**ffmpeg는 파일을 열 때 정확히 `Range: bytes=0-`를 보낸다.** 그래서 Qt(FFmpeg
백엔드)에 고화질 URL을 주면 언제나 403이다. `constants.py`의 `_PROBE_RANGE`
주석이 적어 둔 "열린 범위 403"과 같은 현상인데, 고화질에서는 간헐적이 아니라
**항상** 그렇다. 앱이 "전체를 받아 두고 재생"(`_run_merge`)으로 갈 수밖에
없었던 이유가 이것이다.

제약이 하나 더 있다. **요청당 허용 바이트 상한이 포맷마다 다르다** — 1080p(137)는
2MB 요청에 206을 주지만 오디오(140)는 2MB에 403, 128KB에 206을 준다. 비트레이트에
비례하는 상한으로 보인다.

### 해결 — 로컬 중계 + 실시간 remux

`infrastructure/streaming/relay.py`가 127.0.0.1에 중계 서버를 띄운다. 재생기의
**열린 범위** 요청을 받아, 상위로는 **경계 있는 작은 조각**으로 되묻어 이어
붙인다. 재생기 입장에서는 평범한 로컬 파일이다. `/play.mp4`는 ffmpeg를 띄워
영상+오디오를 `-c copy`로 합쳐 fragmented mp4로 흘린다.

실측: 1080p 첫 바이트까지 **0.97초**, 3MB까지 1.58초. 전체(245MB)를 받지 않는다.

### 밟은 함정 셋

1. **조각을 줄여도 크기가 안 줄었다.** `stop = min(pos + chunk - 1, end)` 라
   남은 길이가 조각보다 짧으면 `chunk`를 반으로 줄여도 실제 요청 크기는 그대로다.
   403 → 축소 → 같은 요청 → 403 의 무한 루프가 됐다.
2. **포기할 때 클라이언트를 매달아 뒀다.** `Content-Length`를 이미 약속한 뒤
   조용히 돌아가면 재생기는 남은 바이트를 **영원히 기다린다**(실측: 요청이
   타임아웃까지 멈춤). 연결을 끊어 "끊겼다"를 즉시 알려야 상위가 재시도한다.
3. **일시적 거부를 크기 문제로 오해했다.** 통하던 크기가 403을 받으면 그것은
   속도 제한이지 상한 문제가 아니다. 줄이면 요청이 더 잦아져 악화된다 — 실측으로
   seek 첫 바이트가 1초대에서 22초로 나빠졌다. 그래서 `StreamSource.proven`으로
   "한 번이라도 통한 크기인가"를 구분해, 통하던 크기면 **쉬었다 같은 크기로**
   재시도한다.

### 남은 제약 — 먼 오프셋은 원본이 거부한다

seek 구현 자체는 정확하다(ffmpeg가 `bytes=43655361-`로 올바르게 Range-seek 한다).
그런데 원본이 그 요청을 거부한다:

```
앞 0%     start=        0   헤더Range=206   쿼리range=200
중간 54%  start= 43692479   헤더Range=403   쿼리range=403
뒤 90%    start= 72820799   헤더Range=403   쿼리range=403
```

요청 형태의 문제가 아니라 **먼 오프셋 접근 자체를 막는다**(PO token 없는 URL은
순차 재생만 허용하는 것으로 보인다). 이것은 스트리밍만의 문제가 아니다 — 같은
이유로 **기존의 전체 다운로드도 중간에서 403으로 멈춘다**(실측: yt-dlp 자체
다운로드가 9%에서 403).

그래서 재생이 같은 자리에서 되풀이해 끊기면 `_fall_back_to_merge`가 예전 방식으로
내려간다. 처음부터 순차로 받는 것은 허용되므로, 기다림을 치르고 seek까지 되돌려
받는 쪽을 택한 것이다. 이 판정은 **영상마다 새로 한다**(`_reset_stream_mode`).

### seek을 우리가 맡는다

파이프로 흘리는 fragmented mp4는 길이도 없고 되감기도 안 된다. 그래서:

- 길이는 yt-dlp 값(`_stream_duration_ms`)으로 대신 답한다(`_effective_duration`).
  이걸 빼먹으면 진행 막대가 채워진 채 멎는다.
- seek은 `?ss=` 로 **새 연결을 연다**. 재생기 위치는 매번 0부터 다시 세므로
  영상 기준 위치 = 재생기 위치 + `_stream_offset_ms` 다.
- 모든 seek 진입점을 `_seek_to` 하나로 모으고 300ms 타이머로 합친다 — J/L 연타마다
  ffmpeg를 새로 띄우면 화면이 멎는다.
- 상위가 조각을 거부하면 **중간에서도 EndOfMedia**가 온다. 그대로 믿으면 재생목록이
  멋대로 다음 곡으로 넘어간다 → 끝 근처일 때만 '다 봤다'로 친다.

---

## 설명서는 원본을 하나만 둔다 (v1.32)

GitHub에서 읽히는 마크다운과 앱이 F1으로 여는 HTML을 따로 쓰면 반드시 한쪽이
낡는다. `docs/manual.md`가 원본이고, `scripts/build_manual.py`가 그것을
`docs/manual/index.html`로 만든다.

마크다운 라이브러리를 새로 들이지 않았다 — 설명서가 쓰는 문법(제목·문단·목록·표·
인용·수평선·이미지·링크·굵게·인라인 코드)만 다루면 충분하고, 그만큼은 의존성을
늘릴 값어치가 없다. 대신 **다루지 못하는 문법은 조용히 글자로 나오므로**
(`| 키 | 동작 |`이 표가 아니라 문장으로 보이는 식) `tests/unit/test_build_manual.py`가
실제 설명서를 렌더해 "남은 마크다운이 없는가"와 "목차 링크가 실제 앵커를 가리키는가"를
고정한다.

**F1은 번들된 HTML을 먼저 찾는다.** 설치해 쓰는 앱이라 오프라인에서도 열려야 하고,
무엇보다 GitHub 문서는 이미 다음 버전을 설명하고 있을 수 있다. 번들에 없을 때만
저장소 문서로 보낸다(`gui/help.py`).

화면 갈무리는 `scripts/capture_screenshots.py`가 **임시 DB에 표본 자료를 심고 실제
앱 화면을 그려** 만든다. 손으로 찍으면 화면이 바뀔 때 조용히 낡고, 사용자의 실제
라이브러리가 찍힐 위험도 있다.
...
v1.32.0 설명서 갈무리에 **사용자의 실제 즐겨찾기 항목**이 찍혀 공개 저장소와 설치본에
들어갔다. 갈무리 스크립트는 임시 DB를 쓰고 있었는데도 그랬다.

이유는 이 앱에 사용자 자료가 사는 곳이 **셋**이기 때문이다.

| 무엇 | 어디 |
| --- | --- |
| 영상·다운로드 이력 등 | `config.settings.DATABASE_PATH` (= `DATA_DIR/library.db`) |
| 설정(테마·태그·브라우저 프로필 경로) | `DATA_DIR/config.yaml` |
| **즐겨찾기 바 항목** | **OS 사용자 데이터 경로** (`platformdirs.user_data_dir`) |

DB만 갈아끼우면 나머지 둘은 그대로 사용자 것을 읽는다. 특히 세 번째는 `DATA_DIR`
밖이라 "데이터 디렉터리를 옮긴다"는 발상으로도 잡히지 않는다 — 패키징 규칙("사용자
데이터는 `platformdirs`로")을 그 파일만 따르고 있어서 생긴 균열이다.

**해결**: `OVC_DATA_DIR` 환경 변수 하나를 세 곳이 함께 본다. 갈무리 스크립트가 그것만
세우면 DB·설정·즐겨찾기가 모두 샌드박스로 따라온다.

밟은 함정 둘:

1. **읽는 시점이 중요하다.** 경로 상수는 `config.settings`를 **불러올 때** 정해진다.
   스크립트 위쪽에 앱 모듈 임포트가 한 줄이라도 있으면 그 시점에 실제 경로가 굳어
   격리가 **조용히** 깨진다. 그래서 스크립트는 맨 위에서 환경 변수를 세우고 무거운
   임포트를 전부 `main()` 안으로 미루며, AST 검사가 이를 고정한다.
2. **샌드박스를 시스템 임시 폴더에 두면 다른 것이 샌다.** 윈도우의 `%TEMP%`는 사용자
   홈 아래라, 설정 화면이 저장 경로를 그대로 보여 주면서 이번엔 **사용자 이름**이 찍혔다.
   샌드박스를 `build/manual-sandbox/`(프로젝트 안)로 옮겨 해결했다.

회귀 시험은 `tests/unit/test_capture_isolation.py`다. 환경 변수를 읽는 곳을 하나라도
빼면 실패한다(실제로 빼 보고 확인했다). 변수가 없을 때 기본 경로가 그대로인지도 함께
고정한다 — 사용자 데이터가 엉뚱한 곳으로 옮겨지는 것이 더 큰 사고다.

덧붙여 다운로드 화면 갈무리가 **완전히 비어** 있었다. 표본 자료를 영상에만 심고
`download_history`에는 심지 않아서다. 빈 화면은 설명서에서 쓸모가 없다.
