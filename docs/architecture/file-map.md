...
│   │   ├── win32_hook.py            # `WM_NCCALCSIZE`로 프레임 영역만 0으로 만들고 `WM_NCHITTEST`로 테두리·캡션을 알려 준다. **`WS_THICKFRAME | WS_CAPTION`을 남겨 두는 것이 핵심** — 그래야 Aero Snap·창 그림자·8방향 리사이즈·최대화 애니메이션·`Win+화살표`·시스템 메뉴가 전부 OS 기본으로 들어오고, 드래그 코드를 한 줄도 쓰지 않는다. 최대화 시 프레임 두께만큼 되줄이지 않으면 사방이 화면 밖으로 나간다(우측 스크롤바가 잘린다) — **전체화면에는 적용하지 않는다**(검은 띠가 남는다). 자동숨김 작업표시줄 변에는 2px 양보(100% 덮으면 표시줄이 다시 안 나온다). 최대화 판정은 Qt `isMaximized()`가 아니라 `GetWindowPlacement`(메시지 처리 시점엔 Qt 상태가 아직 안 바뀌었을 수 있다). `ctypes`만 쓴다(pywin32 미사용 — PyInstaller hidden-import 회피)
